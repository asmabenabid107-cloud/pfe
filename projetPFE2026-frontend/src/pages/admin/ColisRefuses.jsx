...
export default function ColisRefuses() {
  ...
  const loadColis = async () => {
    ...
    try {
      ...
      setColisList(all.filter(c => c.admin_note === "refusé"));
    } catch {
  ...
  return (
    <div style={{ padding: "24px 28px", fontFamily: "system-ui, Arial, sans-serif", color: "var(--text-primary)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "1.3rem", fontWeight: 900 }}>❌ Colis refusés</h1>
          <p style={{ margin: "4px 0 0", opacity: 0.6, fontSize: "0.85rem" }}>
            {loading ? "Chargement..." : `${colisList.length} colis refusés`}
          </p>
        </div>
        <button onClick={loadColis} style={{ background: "rgba(255,95,95,.15)", border: "1px solid var(--danger-border)", color: "var(--danger)", borderRadius: 10, padding: "10px 18px", cursor: "pointer", fontWeight: 700 }}>
          🔄 Rafraîchir
        </button>
      </div>
      ...
      <div style={{ marginBottom: 20, padding: "16px 20px", borderRadius: 14, background: "var(--danger-bg)", border: "1px solid rgba(255,95,95,.25)", display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div>
          <div style={{ fontWeight: 800, color: "var(--danger)" }}>Colis refusés par l'admin</div>
          <div style={{ opacity: 0.6, fontSize: "0.82rem" }}>Ces colis ont été refusés et marqués comme annulés</div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(colis => {
            const st       = STATUS_STYLES[colis.statut] || STATUS_STYLES.annulé;
                  ...
                  <div style={{ borderTop: "1px solid var(--border-subtle)", padding: "16px 20px", background: "var(--surface-inset-strong)" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 16 }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: "0.88rem" }}>
                          ...
                          <div><span style={{ opacity: 0.5 }}>Poids : </span><strong>{colis.poids} kg</strong></div>
                          <div><span style={{ opacity: 0.5 }}>Prix : </span><strong style={{ opacity: 0.5, textDecoration: "line-through" }}>{colis.prix} DT</strong></div>
                          <div><span style={{ opacity: 0.5 }}>Créé le : </span>{colis.created_at ? new Date(colis.created_at).toLocaleString("fr-TN") : "—"}</div>
                          <div style={{ marginTop: 4, padding: "8px 12px", borderRadius: 8, background: "rgba(255,95,95,.1)", border: "1px solid rgba(255,95,95,.25)", color: "var(--danger)", fontSize: "0.82rem", fontWeight: 700 }}>
                            ❌ Ce colis a été refusé et annulé par l'admin
                          </div>
                        </div>
                      </div>
                    </div>
                    {produits.length > 0 && (
                      <div>
                        ...
                        <div style={{ borderRadius: 10, overflow: "hidden", border: "1px solid var(--border-subtle)", opacity: 0.7 }}>
                          ...
                          {produits.map((p, i) => (
                            <div key={i} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 1fr", padding: "10px 14px", borderTop: "1px solid rgba(255,255,255,.06)" }}>
                              ...
                              <div style={{ fontWeight: 800, color: "var(--danger)" }}>{((Number(p.quantite)||0)*(Number(p.prix)||0)).toFixed(2)} DT</div>
                            </div>
                          ))}
                        </div>
                      </div>
